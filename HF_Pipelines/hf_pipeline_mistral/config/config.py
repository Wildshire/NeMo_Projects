...
def _load_model(model_name_or_path, device, num_gpus, hf_auth_token=None, debug=False):
    ...
    if hf_auth_token is None:
        tokenizer = AutoTokenizer.from_pretrained(model_name_or_path, use_fast=False,cache_dir="/.cache/huggingface/hub/", local_files_only=LOCAL_FILES_ONLY)
        model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, low_cpu_mem_usage=True, cache_dir="/.cache/huggingface/hub/", local_files_only=LOCAL_FILES_ONLY,**kwargs
        )
    else:
        tokenizer = AutoTokenizer.from_pretrained(
            model_name_or_path, use_auth_token=hf_auth_token, use_fast=False, cache_dir="/.cache/huggingface/hub/", local_files_only=LOCAL_FILES_ONLY
        )
        model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path,
            low_cpu_mem_usage=True,
            use_auth_token=hf_auth_token,
            cache_dir="/.cache/huggingface/hub/", local_files_only=LOCAL_FILES_ONLY,
            **kwargs,
        )
...
def init_main_llm(config: RailsConfig):
    ...
    provider = get_llm_instance_wrapper(
        llm_instance=hf_llm, llm_type="hf_pipeline_mistral_7b"
    )
    register_llm_provider("hf_pipeline_mistral_7b", provider)
